...
export function usePosts() {
	...
	const fetchMyPosts = useCallback(
		async (params?: { status?: string; limit?: number; nextToken?: string }) => {
	...
	const loadMore = useCallback(async () => {
		if (state.nextToken) {
			await fetchMyPosts({ nextToken: state.nextToken })
		}
...
export function usePost(_slug?: string) {
	...
	const createPost = useCallback(async (input: CreatePostInput) => {
		setState((prev) => ({ ...prev, isSaving: true, isUploading: true, error: null }))
		...
		try {
			...
			setState((prev) => ({
				...prev,
				post: { ...response.data, content: input.content },
				isSaving: false,
			}))
			...
			setState((prev) => ({ ...prev, error: message, isSaving: false, isUploading: false }))
			throw err
		}
	}, [])
	...
	const publishPost = useCallback(async (postSlug: string) => {
		...
		try {
			...
			setState((prev) => ({
				...prev,
				post: prev.post ? { ...prev.post, ...response.data } : null,
				isSaving: false,
			}))
			return response.data
		} catch (err) {
